...
## Приклад: тактичний патч проти стратегічного рішення

Потрібно додати спосіб оплати криптовалютою. Тактичний шлях — вставити ще одну гілку `if paymentMethod == "crypto"` у `CheckoutController` і розмістити там же логіку конвертації курсу.

Стратегічний шлях — визначити контракт `PaymentMethod.charge(amount)`, перенести наявні способи оплати в його реалізації й передавати потрібну реалізацію контролеру ззовні. Тоді новий спосіб реалізує той самий контракт, а сценарій оформлення замовлення лишається стабільним. Початкова зміна може зайняти більше часу, зате наступні способи оплати додаються без розростання умов у контролері. Це і є інвестиція в дешевші майбутні зміни, а не обіцянка конкретного виграшу в годинах.

^aphsd-ch03-example-payment-method
