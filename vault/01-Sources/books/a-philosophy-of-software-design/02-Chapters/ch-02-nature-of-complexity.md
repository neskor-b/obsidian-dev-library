...
## Приклад: когнітивне навантаження від прихованої залежності

```python
def send_invoice(order):
    apply_late_fee(order)
    total = order.total  # сума вже містить пеню
    email.send(order.customer, total)
```

У цьому прикладі лист має показувати суму з пенею. Тому `total` треба читати після `apply_late_fee(order)`: перестановка двох рядків надішле застарілу суму. Сигнатура `send_invoice` не розкриває цього правила. Розробник мусить пам'ятати його під час редагування — у цьому й полягає когнітивне навантаження від прихованої залежності.

^aphsd-ch02-example-cognitive-load
