...
## Приклад: URL shortener

Запит «спроєктуй скорочувач URL» потребує уточнень перед вибором компонентів:

- Який профіль навантаження: скільки коротких посилань створюють і скільки переходів виконують за секунду? Лише після оцінки піків можна вирішувати, чи потрібні кеш і розподіл даних.
- Яка затримка створення допустима? Короткий код має бути унікальним до публікації відповідності. Якщо генератор створив колізію, сервіс перевіряє її й повторює спробу або резервує код іншим способом.
- Чи є строк дії посилань і як швидко після нього вони мають перестати відкриватися? TTL визначає політику видалення й поведінку читання; і реляційне, і key-value сховище можуть її реалізувати.
- Чи потрібна аналітика переходів у реальному часі? Від цього залежить, чи потрібен окремий потік подій і обробка статистики.

Наприклад, оцінка високої частки повторних переходів може виправдати кеш гарячих посилань. Вимоги до унікальності, обсягу та строку зберігання допомагають обрати сховище й механізм резервування кодів. Так компоненти випливають із вимог, а не з популярності технологій.

^sdh-system-design-example-url-shortener
