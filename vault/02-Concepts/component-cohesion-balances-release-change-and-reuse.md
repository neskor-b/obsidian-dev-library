...
## Спрощена схема напруги

```mermaid
flowchart TD
    REP["REP: межа компонента і релізу"]
    CCP["CCP: класи, що змінюються разом"]
    CRP["CRP: лише потрібні клієнту класи"]
    REP ---|"спільний реліз і спільна зміна можуть укрупнювати"| CCP
    CRP ---|"звужує залежності клієнта"| REP
    CRP ---|"звужує залежності клієнта"| CCP
```

`REP` вимагає осмисленого спільного релізу, а `CCP` збирає класи зі спільними причинами змін. Разом ці сили можуть укрупнювати компоненти. `CRP` стримує укрупнення, щоб клієнт не залежав від зайвого. Між ними немає фіксованої ідеальної точки: коли змінюються споживачі й спосіб релізу, межі компонентів теж варто переглядати.

^component-cohesion-tension-diagram-simplified
