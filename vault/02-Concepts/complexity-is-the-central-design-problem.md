...
def calculate_price(order):
    ...
    if order.user.is_premium:
        price *= 0.9
    if order.created_at.weekday() >= 5 and not order.user.is_premium:
        price *= 0.95
    return price
```

Функція коротка, але нову знижку не можна безпечно додати без уточнення пріоритетів: чи можна поєднувати акції, чому вихідна знижка виключає premium-користувачів і як рахувати нові правила. Частина умов видима в коді, а їхнє бізнесове обґрунтування — ні. Складність тут полягає в знаннях, потрібних для зміни, а не в кількості рядків.

^complexity-central-design-problem-example

## Ознаки в коді

- Проста на вигляд зміна вимагає врахування багатьох прихованих залежностей або неочевидних правил.
- Щоб бути впевненим у зміні, розробник мусить одночасно утримувати в голові забагато деталей системи.
