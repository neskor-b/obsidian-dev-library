...
Команда хоче додати експорт звіту в PDF. У першій системі формати обираються розгалуженнями одного великого методу; зміна вимагає редагувати цей метод і перевіряти наявні CSV та HTML. У другій системі формати реалізують `ReportRenderer`; команда додає PDF-реалізацію й реєструє її в місці складання застосунку. Скільки часу це займе, залежить і від PDF-бібліотеки, і від тестів, але архітектура визначає, скільки наявного коду доведеться зачепити.

^architecture-cost-of-change-example
